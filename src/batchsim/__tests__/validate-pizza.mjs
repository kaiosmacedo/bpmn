...
test('buildGraph - parses all processes in collaboration', () => {
  ...
  assert(graph.elementsById.size > 0, `has elements, got ${graph.elementsById.size}`);
  assert(graph.startEvents.length >= 2, `has ≥2 start events (collaboration), got ${graph.startEvents.length}: ${graph.startEvents.join(', ')}`);

  // _6-61 is the customer start event
  assert(graph.elementsById.has('_6-61'), '_6-61 (Hungry for pizza) exists in graph');

  // _6-450 is the vendor start event
  assert(graph.elementsById.has('_6-450'), '_6-450 (Order received) exists in graph');
  ...
  assert(gw.$type === 'bpmn:EventBasedGateway', `_6-180 type is EventBasedGateway, got ${gw.$type}`);
  ...
  assert(pg.$type === 'bpmn:ParallelGateway', `_6-652 type is ParallelGateway, got ${pg.$type}`);

  // outgoing flows
  const gwOuts = graph.outgoingById.get('_6-180') || [];
  assert(gwOuts.length === 2, `EventBasedGateway _6-180 has 2 outgoing flows, got ${gwOuts.length}`);
});
...
test('runBatch - pizza collaboration with startEventId=_6-61', async () => {
  ...
  assert(results.eventsRows.length > 0, `has event rows, got ${results.eventsRows.length}`);
  assert(results.summaryRows.length === 3, `has 3 summary rows, got ${results.summaryRows.length}`);
  assert(results.casesRows.length > 0, `has case rows, got ${results.casesRows.length}`);

  // Check that tokens actually proceed past the start event (no token_error)
  const errors = results.eventsRows.filter(e => e.eventType === 'token_error');
  ...
  assert(completedCases.length > 0, `has completed cases, got ${completedCases.length}`);

  // Check cycle times exist
  const withCycleTime = completedCases.filter(c => c.cycleTime != null && c.cycleTime > 0);
  assert(withCycleTime.length > 0, `has cases with cycle time, got ${withCycleTime.length}`);

  console.log(`    Events: ${results.eventsRows.length}`);
  console.log(`    Completed cases: ${completedCases.length}/${results.casesRows.length}`);
  if (withCycleTime.length > 0) {
    const avg = withCycleTime.reduce((a, c) => a + c.cycleTime, 0) / withCycleTime.length;
...
test('computeMetrics - pizza collaboration results', async () => {
  ...
  assert(metrics.completedCases > 0, `completed cases > 0, got ${metrics.completedCases}`);
  assert(metrics.avgCycleTime > 0, `avg cycle time > 0, got ${metrics.avgCycleTime}`);
...
test('buildStructuredLog - pizza collaboration', async () => {
  ...
  const rngFactory = (seed) => mulberry32(seed);
  const results = await runBatch({ graph, cfg: smallCfg, rng: rngFactory });
  const log = buildStructuredLog(results.eventsRows, graph.elementsById, smallCfg);
